...
readFile("assets/day13.txt", { encoding: 'utf8' })
...
export function draw_grid(grid: IHash)
{
    for (let y = 0; y <= 19; y++)
    {
        let s = "";
        for (let x = 0; x <= 43; x++)
        {
            ...
            if (key in grid && grid[key] === Tiles.Block)
                s += "@";
            else if (key in grid && grid[key] === Tiles.Ball)
                s += "*";
            else if (key in grid && grid[key] == Tiles.Paddle)
                s += "^";
            else if (key in grid && grid[key] == Tiles.Wall)
                s += "|";
            else
                s += " ";
        }
...
export default function run(data: string)
{
    ...
    while (!computer.stopped)
    {
        ...
        if (computer.output_stream.length)
        {
            ...
            {
                // X phase:
                x_output = strint_to_num(instruction);
                x_max = Math.max(x_max, x_output);
                x_min = Math.min(x_min, x_output);
                phase++;
            }
            else if (phase === 1)
            {
                // Y phase:
                y_output = strint_to_num(instruction);
                y_max = Math.max(y_max, y_output);
                y_min = Math.min(y_min, y_output);
                phase++;
            }
            else if (phase === 2)
            {
                // Tile phase:
                let key = String(x_output) + "_" + String(y_output);
                grid[key] = strint_to_num(instruction);
                if (instruction === Tiles.Ball)
                    ball_x = x_output;
                else if (instruction === Tiles.Paddle)
                    paddle_x = x_output;
                phase = 0;
            }
        }
