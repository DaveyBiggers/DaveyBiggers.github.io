...
function create_lines(wires) {
    let segments = wires.split(",");
    let x = 0;
    let y = 0;
    let lines = [];
    for (let segment of segments) {
        let xmul = segment.startsWith("R") ? 1 : segment.startsWith("L") ? -1 : 0;
        let ymul = segment.startsWith("U") ? -1 : segment.startsWith("D") ? 1 : 0;
        let distance = parseInt(segment.substr(1));
...
function run(data) {
    ...
    let linesA = create_lines(wires[0]);
    let linesB = create_lines(wires[1]);
    console.log("Requires:", linesA.length * linesB.length, "intersection tests.");
    let min_intersection_distance = -1;
